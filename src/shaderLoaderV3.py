...
def compile_shader(vs, fs):
    vert_shader_source = load_shader(vs)
    frag_shader_source = load_shader(fs)

    shader_program = OpenGL.GL.shaders.compileProgram(
        OpenGL.GL.shaders.compileShader(vert_shader_source, GL_VERTEX_SHADER),
        OpenGL.GL.shaders.compileShader(frag_shader_source, GL_FRAGMENT_SHADER)
    )
